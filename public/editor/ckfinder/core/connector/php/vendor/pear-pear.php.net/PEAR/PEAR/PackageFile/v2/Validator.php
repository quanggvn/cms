...
class PEAR_PackageFile_v2_Validator
{
    ...
    function _stupidSchemaValidate($structure, $xml, $root)
    {
        ...
        if (!$mismatch && count($optionaltags)) {
            // don't errors out on any optional tags
    ...
    function _invalidInternalStructure()
    {
        $this->_stack->push(__FUNCTION__, 'exception', array(),
            'internal array was not generated by compatible parser, or extreme parser errors, cannot continue');
    }
    ...
    function analyzeSourceCode($file, $string = false)
    {
        if (!function_exists("token_get_all")) {
            $this->_stack->push(__FUNCTION__, 'error', array('file' => $file),
                'Parser errors: token_get_all() function must exist to analyze source code, PHP may have been compiled with --disable-tokenizer');
            return false;
        }
        ...
        if (isset($php_errormsg)) {
            if (isset($this->_stack)) {
                ...
                $this->_stack->push(__FUNCTION__, 'warning',
                        ...
                        ' possibly a PHP parse errors in %file% from %package%');
            }
        }
/*
                ...
                case T_CLASS:
                    if (($current_class_level != -1) || ($current_function_level != -1)) {
                        if (isset($this->_stack)) {
                            ...
                            PEAR::raiseError("Parser errors: invalid PHP found in file \"$file\"",
